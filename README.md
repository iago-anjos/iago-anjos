### Hi there, I'm Iago! 👋

<p align="center">
  <a href="https://github.com/iago-anjos">
    <img src="https://github-readme-stats.vercel.app/api?username=iago-anjos&show_icons=true&theme=dracula&include_all_commits=true&count_private=true" alt="Iago's GitHub Stats"/>
  </a>
  <a href="https://github.com/iago-anjos">
    <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=iago-anjos&layout=compact&langs_count=8&theme=dracula" alt="Iago's Top Languages"/>
  </a>
</p>

---

👨‍💻 Full-Stack Developer based in **Brazil** 🇧🇷, building high-performance e-commerce on **VTEX** and **Nuvemshop**.

I work end-to-end on online stores — from custom storefronts in **React** and **Next.js** to backend integrations in **Node.js** with REST and GraphQL APIs. Focused on writing clean, scalable code and delivering shopping experiences that actually convert.

🌍 Open to remote opportunities, including positions in the European tech scene.

📫 **[LinkedIn](https://www.linkedin.com/in/iagokaued/)** • ✉️ **seu@email.com**

---

### 🛠️ Tech Stack

**Languages & Core**

<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=js,ts,html,css,nodejs" />
  </a>
</p>

**Frontend**

<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=react,nextjs,styledcomponents,tailwind" />
  </a>
</p>

**Backend & APIs**

<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=express,graphql,postman" />
  </a>
</p>

**E-commerce Platforms**

<p align="left">
  <img src="https://img.shields.io/badge/VTEX-F71963?style=for-the-badge&logo=vtex&logoColor=white" alt="VTEX"/>
  <img src="https://img.shields.io/badge/VTEX_IO-F71963?style=for-the-badge&logoColor=white" alt="VTEX IO"/>
  <img src="https://img.shields.io/badge/FastStore-000000?style=for-the-badge&logo=next.js&logoColor=white" alt="FastStore"/>
  <img src="https://img.shields.io/badge/Nuvemshop-1572B6?style=for-the-badge&logoColor=white" alt="Nuvemshop"/>
</p>

**Tools**

<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=git,github,vscode,figma,npm" />
  </a>
</p>

---

### 🚀 What I'm working on

- 🛒 Building custom storefronts on **VTEX IO** (Store Framework & FastStore)
- 🎨 Developing themes and apps for **Nuvemshop**
- 🔌 Integrating ERPs, payment gateways and marketplaces via REST & GraphQL

### 📚 Currently learning

- Diving deeper into **VTEX IO** — builders, Master Data and custom GraphQL apps
- **FastStore** with Next.js for projects focused on Core Web Vitals
- Composable commerce best practices and architecture patterns

---

### ⚡ Recent GitHub Activity

<!--START_SECTION:activity-->
<!--END_SECTION:activity-->
